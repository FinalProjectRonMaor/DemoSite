...
export class AppComponent implements OnInit, AfterContentInit {
  ...
  constructor(private dataStorageService : DataStorageService, private formSubmitService: FormSubmitService, private http: HttpClient, private aiService: ApiService, private gptService: GptService) {}
...
// send form to API
  ...
      }
    ...
    //sending the response to chat gpt-
    this.loader = true;
    this.gptService.sendMessage(
      `Act as a group of psychologist, data analyst and web designer
       your assignment is to receive the following data we extracted by getting from the user his mood
       we requested from the user the following data:
       happiness level from 1-6,
       anger level from 1-6,
       stress level from 1-6
       tired level from 1-6
       and distracted level from 1-6
       we need you to answer without introducing yourself in th following pattern:
       "<b>Analysis:</b>" summarize and conclude his mood and what we can deduce from that information
       "<br><b>Implementations:</b>" in this section we want you to write an explanation on how this data can be used to make an adaptable web app
       that will suite the user's needs, give examples like: for an e-comers site we can make the products more approachable
       because the user is stressed and happy at the same time so we can indicate that he is eager to buy something, 
       for a consulting site i will make the site less flashy because the user is upset etc'
       also you can add rich text tags so I could present the response with line breaks bolded text etc'
       for example after the analysis and before Implementations add a line break tags; <br> and make sure the text "<b>Analysis:</b>" and "<br><b>Implementations:</b>" are bolded

       the data we received from the user is as follows:
       RES: \n
       distractedLevel ${res.distructedLevel[0]}\n
       fearLevel ${res.fearLevel[0]}\n
       joyLevel ${res.joyLevel[0]}\n
       sadnessLevel ${res.sadnessLevel[0]}\n
       stressLevel ${res.stressLevel[0]}\n
      ...
      this.loader = false;
      if (gptRes.choices[0].finish_reason === "stop"){
        console.log(gptRes.choices[0].message.content);
        this.gptService.res = gptRes.choices[0].message.content
      }
