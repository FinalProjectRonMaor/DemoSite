...
import { UserData } from './interfaces/ai-req-data.interface';
import { AiApiResponse } from '../public-api';

@Component({
  selector: 'lib-web-based-sentiment-analysis-api',
  template: ``,
  styles: []
})
export class WebBasedSentimentAnalysisApiComponent implements OnInit {
  ...
  constructor(private aiService: ApiService) { }
  ...
  getPrediction(form: UserData, apiUrl: string): Observable<AiApiResponse> {
    return this.aiService.predict(
      {
        "data": {
            "User's Age": form.userAge,
            "User's Sex": form.userSex === 'male'? 0 : form.userSex === 'Male'?  0 : form.userSex === 'm'? 0 : form.userSex === 'M'? 0 : 1,
            "Speed": this.avgPixelPerSecond,
            "Dominant Hand": form.dominantHand === 'right'? 0 : form.dominantHand === 'Right'? 0 : form.dominantHand === 'r'? 0 : form.dominantHand === 'R'? 0 : 1 ,
            "Backspaces Count": this.BackspaceComparedToTyping(this.typeKeyBoardKeyArray),
